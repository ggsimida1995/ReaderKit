    ...
    /// 更新字体
     func updateFont() {
        ...
        if !NSDictionary(dictionary: attributes).isEqual(to: tempAttributes) {
            ...
            pageModels = DZMReadParser.pageing(attrString: fullContent, rect: CGRect(origin: CGPoint.zero, size: DZM_READ_VIEW_RECT.size), isFirstChapter: isFirstChapter, isContentEmpty: isContentEmpty)

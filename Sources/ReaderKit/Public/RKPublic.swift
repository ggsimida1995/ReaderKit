...
var DZM_READ_VIEW_RECT:CGRect! {
     // 页眉高度
    let headerHeight = ReaderLayoutManager.shared.headerHeight
    // 页脚高度
    let footerHeight = ReaderLayoutManager.shared.footerHeight
    // 上边距 如果是滚动则没有上边距和下边距

    let top = ReaderLayoutManager.shared.effectType == .scroll ?  0 : ReaderLayoutManager.shared.pageInsets.top
    // 下边距
    let bottom = ReaderLayoutManager.shared.effectType == .scroll ? 0 : ReaderLayoutManager.shared.pageInsets.bottom
    // 左边距
    let left = ReaderLayoutManager.shared.pageInsets.left
    // 右边距
    let right = ReaderLayoutManager.shared.pageInsets.right
   
    

    let rect = DZM_READ_RECT!
    
    return CGRect(x: rect.minX + left, y: rect.minY + headerHeight + top, width: rect.width - left - right, height: rect.height - headerHeight - footerHeight - top - bottom)
}
...
/// 计算总进度
func DZM_READ_TOTAL_PROGRESS(readModel: RKReadModel, recordModel: RKReadRecordModel) -> Float {
    // 当前阅读进度

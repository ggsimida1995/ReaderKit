...
/// 监控阅读长按视图通知
func _READ_NOTIFICATION_MONITOR(target:Any, action:Selector) {
    
    NotificationCenter.default.addObserver(target, selector: action, name: NSNotification.Name(_READ_NOTIFICATION_LONG_PRESS_VIEW), object: nil)
}

/// 发送阅读长按视图通知
func _READ_NOTIFICATION_PUSH(userInfo:[AnyHashable : Any]?) {
    
    NotificationCenter.default.post(name: NSNotification.Name(rawValue: _READ_NOTIFICATION_LONG_PRESS_VIEW), object: nil, userInfo: userInfo)
}

/// 移除阅读长按视图通知
func _READ_NOTIFICATION_REMOVE(target:Any) {
    
    NotificationCenter.default.removeObserver(target, name: NSNotification.Name(rawValue: _READ_NOTIFICATION_LONG_PRESS_VIEW), object: nil)
}

/// 长按阅读视图通知 info 数据 key
let _READ_KEY_LONG_PRESS_VIEW:String = "_READ_KEY_LONG_PRESS_VIEW"

/// 长按阅读视图通知
let _READ_NOTIFICATION_LONG_PRESS_VIEW:String = "_READ_LONG_PRESS_VIEW_NOTIFICATION"

/// 光标拖拽触发范围
let _READ_LONG_PRESS_CURSOR_VIEW_OFFSET:CGFloat = -20

import UIKit

class RKReadLongPressView: RKReadView {
    ...
    /// 左光标
    private var LCursorView:RKReadLongPressCursorView!
    
    /// 右光标
    private var RCursorView:RKReadLongPressCursorView!
...
//        }
    ...
    @objc private func longAction(long:UILongPressGestureRecognizer) {
        ...
        if long.state == .began {
            ...
            // 发送通知
            _READ_NOTIFICATION_PUSH(userInfo: [_READ_KEY_LONG_PRESS_VIEW : NSNumber(value: true)])
...
//            magnifierView?.targetPoint = windowPoint
            ...
            // 获得选中区域
            selectRange = RKCoreText.GetTouchParagraphRange(point: point, frameRef: frameRef, content: pageModel.content.string)

            // 获得选中选中范围
            rects = RKCoreText.GetRangeRects(range: selectRange!, frameRef: frameRef, content: pageModel.content.string)
...
//            })
            ...
            if !rects.isEmpty {
                ...
                // 发送通知
                _READ_NOTIFICATION_PUSH(userInfo: [_READ_KEY_LONG_PRESS_VIEW : NSNumber(value: false)])
            }
        }
    }
    ...
    /// 拖拽事件解析
    func drag(status:PanGesStatus, point:CGPoint, windowPoint:CGPoint) {
        ...
        if status == .begin {
            ...
            if LCursorView.frame.insetBy(dx: _READ_LONG_PRESS_CURSOR_VIEW_OFFSET, dy: _READ_LONG_PRESS_CURSOR_VIEW_OFFSET).contains(point) { // 触摸到左边光标
            ...
            }else if RCursorView.frame.insetBy(dx: _READ_LONG_PRESS_CURSOR_VIEW_OFFSET, dy: _READ_LONG_PRESS_CURSOR_VIEW_OFFSET).contains(point) { // 触摸到右边光标
...
//                magnifierView?.targetPoint = windowPoint
            ...
            if isTouchCursor && selectRange != nil {
                ...
                // 触摸到的位置
                let location = RKCoreText.GetTouchLocation(point: point, frameRef: frameRef)
    ...
    private func cursor(isShow:Bool) {
        ...
        if isShow {
            ...
            if !rects.isEmpty && LCursorView == nil {
                ...
                LCursorView = RKReadLongPressCursorView()
                LCursorView.isTorB = true
                addSubview(LCursorView)
                
                RCursorView = RKReadLongPressCursorView()
    ...
    private func showMenu(isShow:Bool) {
        ...
        if isShow { // 显示
            ...
            if !rects.isEmpty {
                ...
                let rect = RKCoreText.GetMenuRect(rects: rects, viewFrame: bounds)

...
class RKReadChapterModel: NSObject, NSCoding {
    ...
    /// 分页数据
    var pageModels: [RKReadPageModel] = []
    ...
    /// 当前章节是否为最后一个章节
    var isLastChapter: Bool { nextChapterID == -1 }
    ...
    func updateFont() {
        ...
        if !NSDictionary(dictionary: attributes).isEqual(to: tempAttributes) {
            ...
            pageModels = RKReadParser.pageing(attrString: fullContent, rect: CGRect(origin: .zero, size: DZM_READ_VIEW_RECT.size), isFirstChapter: isFirstChapter, isContentEmpty: isContentEmpty)
    ...
    /// 保存
    func save() {
        RKKeyedArchiver.archiver(folderName: bookID, fileName: id.description, object: self)
    }
    
    /// 是否存在章节内容
    class func isExist(bookID: String, chapterID: Int) -> Bool {
        return RKKeyedArchiver.isExist(folderName: bookID, fileName: chapterID.description)
    }
    
    // MARK: 构造
    
    /// 获取章节对象,如果则创建对象返回
    @objc class func model(bookID: String, chapterID: Int, isUpdateFont: Bool = true) -> RKReadChapterModel {
        var chapterModel: RKReadChapterModel!
        
        if RKReadChapterModel.isExist(bookID: bookID, chapterID: chapterID) {
            chapterModel = RKKeyedArchiver.unarchiver(folderName: bookID, fileName: chapterID.description) as? RKReadChapterModel
            if isUpdateFont { chapterModel?.updateFont() }
        } else {
            chapterModel = RKReadChapterModel()
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        pageModels = aDecoder.decodeObject(forKey: "pageModels") as? [RKReadPageModel] ?? []

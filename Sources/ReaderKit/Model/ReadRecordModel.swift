...
/// 记录当前章节阅读到的坐标
var _READ_RECORD_CURRENT_CHAPTER_LOCATION:NSNumber!
...
class ReadRecordModel: NSObject,NSCoding {
    ...
    /// 当前记录分页模型
    var pageModel:ReadPageModel! { return  chapterModel.pageModels[page.intValue] }
    ...
    /// 修改阅读记录为指定章节位置
    func modify(chapterModel:ReadChapterModel!, page:NSInteger, isSave:Bool = true) {
    ...
    func updateFont(isSave:Bool = true) {
        ...
        if chapterModel != nil {
            ...
            page = chapterModel.page(location: _READ_RECORD_CURRENT_CHAPTER_LOCATION.intValue)

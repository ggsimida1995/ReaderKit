...
class ReadChapterModel: NSObject,NSCoding {
    ...
    /// 当前章节是否为第一个章节
    var isFirstChapter:Bool! { return (previousChapterID == nil) }
    
    /// 当前章节是否为最后一个章节
    var isLastChapter:Bool! { return (nextChapterID == nil) }
    ...
    /// 更新字体
     func updateFont() {
        ...
        if !NSDictionary(dictionary: attributes).isEqual(to: tempAttributes) {
            ...
            // print("内容是否为空: \(isContentEmpty)")
            pageModels = ReadParser.pageing(attrString: fullContent, rect: CGRect(origin: CGPoint.zero, size: DZM_READ_VIEW_RECT.size), isFirstChapter: isFirstChapter, isContentEmpty: isContentEmpty)
    ...
    func locationFirst(page:NSInteger) ->NSNumber {
        // print("获取指定页开始坐标: \(pageModels.count) \(page)")
    ...
    /// 保存
    func save() { KeyedArchiver.archiver(folderName: bookID, fileName: id.stringValue, object: self) }
    ...
    /// 获取章节对象,如果则创建对象返回
    @objc class func model(bookID:String!, chapterID:NSNumber!, isUpdateFont:Bool = true) ->ReadChapterModel {

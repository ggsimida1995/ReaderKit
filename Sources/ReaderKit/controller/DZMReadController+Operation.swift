...
extension DZMReadController {
    ...
    func GetAboveReadRecordModel(recordModel:DZMReadRecordModel!) ->DZMReadRecordModel? {
        ...
        if recordModel.isFirstPage {
            ...
            if isExist || readModel.bookSourceType == .local {
                ...
                fetchChapterContent(bookID: bookID!, chapterID: chapterID as! Int)
    ...
    // 查询数据库章节数据
//    func queryChapterData(bookID:String, chapterID:NSNumber!) {
//        let chapter = try? DatabaseManager.shared.getChapterSync(bkId: Int(bookID)!, index: Int(truncating: chapterID!))
//        // 解析章节数据
//        let chapterModel = DZMReadChapterModel.model(bookID: bookID, chapterID: chapter!.index as NSNumber)
//        
//        print("chapter!.contents.count: \(chapter!.contents.count)")
...
//        print("chapterCount: \(chapterCount)  chapterID: \(chapterID)")

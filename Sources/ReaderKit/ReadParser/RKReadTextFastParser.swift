...
class RKReadTextFastParser: NSObject {
    ...
    /// - Returns: 阅读对象
    private class func parser(url: URL) -> RKReadModel? {
        // 链接不为空且是本地文件路径
        ...
        if !RKReadModel.isExist(bookID: bookID) { // 不存在
            // 解析数据
            let content = RKReadParser.encode(url: url)
            ...
            return RKReadModel.model(bookID: bookID)
        }
    }
    ...
    ///   - readModel: readModel
    ///   - content: 小说内容
    private class func parser(readModel: RKReadModel, content: String) {
        // 章节列表
        ...
        // 排版
        let content = RKReadParser.contentTypesetting(content: content)
        ...
        if !results.isEmpty {
            ...
            for i in 0...count {
                ...
                // 章节列表
                let chapterListModel = RKReadChapterListModel()
    ...
    /// 获取单个指定章节
    class func parser(readModel: RKReadModel, chapterID: Int, isUpdateFont: Bool = true) -> RKReadChapterModel? {
        // 检查是否存在章节内容
        let isExist = RKReadChapterModel.isExist(bookID: readModel.bookID, chapterID: chapterID)
        ...
        if isExist || readModel.bookSourceType == .local {
            ...
            if !isExist {
                ...
                // 本地存在章节，取出来使用
                return RKReadChapterModel.model(bookID: readModel.bookID, chapterID: chapterID)
            }
        }
